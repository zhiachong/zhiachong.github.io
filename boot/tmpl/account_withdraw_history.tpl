...
<form method=post action="account_withdraw_history.php">
<table cellspacing=0 cellpadding=0 border=0 width=100%>
<tr>
 <td><h3>Withdrawals History:</h3></td>
 <td align=right>
  From: 
...
{/section}
  ...
  {/section}
  </select><br><img src=images/q.gif width=1 height=4>
  <br>
  To: 
...
<option value={$smarty.section.month_to.index+1} {if $smarty.section.month_to.index+1 == $frm.month_to}selected{/if}>{$month[month_to]}
 ...
 <td>&nbsp; <input type=submit value="Go" class=sbmt></td>
</tr>
</table>
</form>

{if $frm.say eq 'canceled'}<h3>Your withdraw request has been canceled.</h3> <br><br>{/if}
...
<tr>
 ...
 <td width=200 align=right><b>{$currency_sign} {$trans[trans].actual_amount}</b> <img src="images/pay/{$trans[trans].ec}.gif" align=absmiddle hspace=1 height=21> {if $trans[trans].transtype eq 'Withdrawal request'} <a href=?a=cancelwithdraw&id={$trans[trans].id} onclick="return confirm('Are you sure you want to delete this request?')">[cancel]</a>{/if}</td>
