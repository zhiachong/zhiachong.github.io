...
/***********************************************************************/
  ...
  if ($frm['a'] == 'cancelwithdraw')
  {
    $trans_id = sprintf ('%d', $frm['id']);
    $q = 'DELETE FROM hm2_history WHERE id = ' . $trans_id . ' AND type=\'withdraw_pending\' AND user_id =' . $userinfo['id'];
    mysql_query ($q);
    header ('Location: account_withdraw_history.php?say=canceled');
    db_close ($dbconn);
    exit ();
  }
  ...
  if ($settings['use_history_balance_mode'])
  {
    ...
    $q = 'select SUM(actual_amount * (actual_amount < 0)) as debit, SUM(actual_amount * (actual_amount > 0)) as credit,
SUM(actual_amount) as balance FROM hm2_history where ' . $datewhere . ' ' . $typewhere . ' and user_id = ' . $userinfo['id'];
    $sth = mysql_query ($q);
    $row = mysql_fetch_array ($sth);
    $start_balance = $row['balance'];
  ...
  $q = 'SELECT SUM(actual_amount) as periodsum from hm2_history where ' . $datewhere . ' and user_id = ' . $id . ' and type in (\'withdrawal\', \'withdraw_pending\')';
  $sth = mysql_query ($q);
  $row = mysql_fetch_array ($sth);
  $periodsum = $row['periodsum'];
  $smarty->assign ('periodsum', number_format (0 - $periodsum, 2));
  $q = 'SELECT SUM(actual_amount) as sum from hm2_history where 1=1 ' . $userwhere . ' and user_id = ' . $id . ' and type in (\'withdrawal\', \'withdraw_pending\')';
  $sth = mysql_query ($q);
  $row = mysql_fetch_array ($sth);
  $allsum = $row['sum'];
  ...
  $smarty->display ('account_withdraw_history.tpl');
?>